...
get_pte(pde_t *pgdir, uintptr_t la, bool create)
{
  ...
  if (!(*pdep & PTE_P))
  {
    ...
    // 3. 如果需要创建，刚才已经 alloc_page 了一个页
    set_page_ref(page, 1);        // 设置引用计数
    uintptr_t pa = page2pa(page); // 得到该页的物理地址
    memset(KADDR(pa), 0, PGSIZE); // 把这一页清空（变成空的二级页表）

    // 4. 更新页目录项，指向这个新创建的二级页表
    // 设置权限：存在(P)、可写(W)、用户可访问(U)
    *pdep = pa | PTE_U | PTE_W | PTE_P;
  }

  // 5. 返回二级页表项的地址
  // PDE_ADDR(*pdep) 取出页目录项中存的物理地址（即二级页表的基址）
  // KADDR(...) 把它转为内核虚拟地址，这样 CPU 才能访问
  // + PTX(la) 加上偏移量，找到具体的那个页表项
...
#define PRINT_PTE(s0, a0, a1, a2, a3, s1) \
  kprintf(s0);                            \
  printhex(a0);                           \
  kprintf(") ");                          \
  printhex(a1);                           \
