...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        // 初始化 run_link，否则会导致 default_sched.c 中的断言失败
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    // 5. 将新进程加入进程列表（需要关中断保护）
    bool intr_flag;
    local_intr_save(intr_flag); // 关中断
    {
        proc->pid = get_pid(); // 获取PID
        hash_proc(proc);       // 加入哈希表
        set_links(proc);       // 设置进程关系并加入proc_list
    }
...
{ // load_icode from binary in kernel file, For LAB5-LAB7
    if (current->mm != NULL)
...
{ // load_icode from disk fd, For LAB8
#ifdef LAB8_EX2
